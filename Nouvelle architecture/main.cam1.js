...
function start(){
	...
	var tabCameras = new Array(8);
		...
		for(var i=0;i<8;i++){
			...
			tabCameras.push(camera);
 	       		...
 	       		tabCameras[0].position.x = -280 ;
			tabCameras[0].position.y = 260 ;
			tabCameras[0].position.z = 50;
		
 	       		tabCameras[1].position.x = 0 ;
			tabCameras[1].position.y = 50 ;
			tabCameras[1].position.z = 100;
			
			tabCameras[2].position.x = 240 ;
			tabCameras[2].position.y = 100 ;
			tabCameras[2].position.z = 80;
			
			tabCameras[3].position.x = 140 ;
			tabCameras[3].position.y = -240 ;
			tabCameras[3].position.z = 80;
			 
			tabCameras[4].position.x = 60 ;
			tabCameras[4].position.y = -40 ;
			tabCameras[4].position.z = 160;
			 
			tabCameras[5].position.x = -100 ;
			tabCameras[5].position.y = -240 ;
			tabCameras[5].position.z = 100;
	...
	function render() { 
		...
		tabCameras[6].position.x = 0.0 ;
		tabCameras[6].position.z = 10.0 ;
		tabCameras[6].position.y = -25.0 ;
		tabCameras[6].rotation.x = 85.0*3.14159/180.0 ;
		car3.add(tabCameras[6]);
		RC.renderer.render(RC.scene,tabCameras[6]);	
	        }
               else if (typeCam == 1){
		tabCameras[7].rotation.x = 0;
		tabCameras[7].position.x = NAV.x;
		tabCameras[7].position.y = NAV.y;
		tabCameras[7].position.z = NAV.z+50+vehicle.speed.length()*2;
		RC.renderer.render(RC.scene,tabCameras[7]);
                }
	        else {

                if(plane==0 || plane==1 || plane==2 || plane==3 || plane==4 ||plane==5 ||plane==29 || plane==28){

		        tabCameras[0].up = new THREE.Vector3(0,0,1);
			tabCameras[0].lookAt (car0.position); 
                        RC.renderer.render(RC.scene,tabCameras[0]);
                    }


                 else if(plane==6 || plane ==7 || plane == 8|| plane==9 || plane == 10){
                        tabCameras[1].up = new THREE.Vector3(0,0,1);
			tabCameras[1].lookAt (car0.position); 
			RC.renderer.render(RC.scene,tabCameras[1]);}


		else if (plane==11 || plane == 12|| plane == 13 || plane==14){
			tabCameras[2].up = new THREE.Vector3(0,0,1);
			tabCameras[2].lookAt (car0.position); 
			RC.renderer.render(RC.scene,tabCameras[2]);}


		else if( plane==15 ||plane == 16|| plane==17){
			tabCameras[3].up = new THREE.Vector3(0,0,1);
			tabCameras[3].lookAt (car0.position); 
			RC.renderer.render(RC.scene,tabCameras[3]);}
		else if( plane == 18 || plane==19 || plane == 20|| plane == 21|| plane==22 ){
			tabCameras[4].up = new THREE.Vector3(0,0,1);
			tabCameras[4].lookAt (car0.position); 
			RC.renderer.render(RC.scene,tabCameras[4]);}
		else {
			tabCameras[5].up = new THREE.Vector3(0,0,1);
			tabCameras[5].lookAt (car0.position); 
			RC.renderer.render(RC.scene,tabCameras[5]);}
		}
