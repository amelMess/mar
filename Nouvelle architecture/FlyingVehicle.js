...
function FlyingVehicle(configuration)
{
	...
	this.lapNumber = 0;
	this.currentPlaneName = null;
	this.lastPlaneName = configuration.lastPlaneName;
	// The navPlane that begins the lap
	this.firstPlaneName = configuration.firstPlaneName;
	...
	this.updateLap = function(activePlaneName) {
		if (!this.lastPlaneName || !this.firstPlaneName || !activePlaneName) {
			return;
		}
		
		var previousPlaneName = this.currentPlaneName;
		this.currentPlaneName = activePlaneName;
		
		// lastPlane > firstPlane
		if (previousPlaneName == this.lastPlaneName && this.currentPlaneName == this.firstPlaneName) {
			this.lapNumber++;
		...
		// firstPlane > lastName
		else if (previousPlaneName == this.firstPlaneName && this.currentPlaneName == this.lastPlaneName) {
			this.lapNumber--;
